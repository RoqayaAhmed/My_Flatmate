...
$query = "SELECT * FROM approved_apartments WHERE id = $id";
    ...
    $rows = $runquery->fetch_assoc();

    $insertquery = "INSERT INTO `cart`( `user_id`, `room_no`, `size`, `price`, `District`,`room_img`) VALUES ('$_SESSION[id]','$rows[room_no]',$rows[size],$rows[price],'$rows[district]','$rows[room_img]')";
